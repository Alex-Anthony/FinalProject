...
async def on_message(message):
   ...
   if user_message.startswith('start tictactoe'):
       response = responses.start_tic_tac_toe(message.channel.id)
       await message.channel.send(response)
       return


   if user_message.startswith('move'):
       try:
...
def run_discord_bot():
   Token = 'MTE4MTc1NDYzMDIwNDU1OTQ1MQ.GW-Pdj.HB3ubeksGCYXGO32fb058sKbySMID9eQMn8yoQ'
   client.run(Token)
